...
import { weeklyDashboard0906 } from "../data/weeklyDashboard";
import { BondIssuanceChart, Sparkline, WeeklyLineChart } from "./WeeklyCharts";

describe("weekly dashboard charts", () => {
  afterEach(cleanup);

  it("plots the 2026 flight line only through the September report date and labels its YoY change", () => {
    const chart = weeklyDashboard0906.heroCharts.find((item) => item.id === "domestic-flights");
    ...
    expect(screen.getByText("周同比 +2.1%")).toBeInTheDocument();
    const currentLine = container.querySelector('[data-series-label="2026"]');
    expect(currentLine).toHaveAttribute("data-last-index", "35");
    expect(Number(currentLine?.getAttribute("data-last-x"))).toBeLessThan(450);
  });

  it("renders the 2024, 2025 and 2026 seasonal activity lines", () => {
    const chart = weeklyDashboard0906.activityCharts[0];
    const { container } = render(<WeeklyLineChart chart={chart} />);
  ...
  it("separates the year, month and week bond comparisons", () => {
    const chart = weeklyDashboard0906.heroCharts.find((item) => item.id === "bond-issuance");
    if (chart?.kind !== "bonds") throw new Error("Expected a bond chart");
    const { container } = render(<BondIssuanceChart chart={chart} />);

    expect(screen.getByRole("img", { name: chart.title })).toBeInTheDocument();
    expect(screen.getByText("本年（1月1日至9月6日）")).toBeInTheDocument();
    expect(screen.getByText("本月（9月1日至9月6日）")).toBeInTheDocument();
    expect(screen.getByText("本周（8月31日至9月6日）")).toBeInTheDocument();
  ...
  it("draws an unlabeled sparkline for the finance table", () => {
    const row = weeklyDashboard0906.financeGroups[0].rows[0];
